...
├── backend/          # FastAPI REST API (Python 3.11) — 15 роутеров, 22 сервиса
├── web/              # Blazor Server фронтенд (.NET 10) — 9 разделов
├── mobile/           # .NET MAUI Android — приложение для бригадиров
├── simulator/        # Симулятор телеметрии 15 котельных (5 сценариев аварий)
├── mock-services/
│   ├── onec-mock/    # Mock 1С REST API (порт 8080)
│   └── edo-mock/     # Mock оператора ЭДО (порт 8081)
├── onec-config/      # Инструкции и БД 1С:Предприятие 8.3
